...
export default function PBHistory() {
  ...
  const filtered = search.trim()
    ? draws.filter(d =>
        d.numbers.some(n => String(n).includes(search.trim())) ||
        String(d.pb).includes(search.trim()) ||
  ...
  return (
    <div className="pb-history">
      <div className="pb-hist-header">
        <div>
          <h1 className="pb-hist-title">🔴 Powerball History</h1>
          <p className="pb-hist-sub">{draws.length} total draws</p>
        </div>
        <input
          className="pb-hist-search"
      ...
      {filtered.length === 0 ? (
        <div className="pb-hist-empty">No draws match your search.</div>
      ) : (
        <>
              ...
              <div key={d.id} className="pb-hist-row">
                ...
                <span className="pb-hist-nums">
                  {d.numbers.map(n => (
                    <span
                      key={n}
                      className={`pb-ball-sm ${search && String(n) === search.trim() ? 'pb-ball-sm-match' : ''}`}
                    >
                      {n}
                    </span>
                  ))}
                </span>
                <span className={`pb-ball-sm-red ${search && String(d.pb) === search.trim() ? 'pb-ball-sm-match-red' : ''}`}>
                  {d.pb}
                </span>

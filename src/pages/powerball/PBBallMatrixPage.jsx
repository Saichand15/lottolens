...
export default function PBBallMatrixPage() {
  ...
  useEffect(() => {
    fetchAllPBDraws()
      .then(data => {
        // Each draw contributes one red Powerball value
        setDraws(data.map(d => [d.pb]))
        setLoading(false)
      })
  ...
  const laserHits = useMemo(() => {
    ...
    return pbComputeLaserHits(displayDraws, selectedCell.colIdx, selectedCell.rowNum, MAX_NUM)
  ...
  if (loading) return <div className="loading-screen"><div className="loading-spinner" /><p className="loading-text">Loading Powerball (1–26) matrix…</p></div>
  ...
  return (
    <div className="matrix-page">
      <div className="matrix-info-bar">
        {lastDraw && (
          <div className="hdr-last">
            <span className="hdr-last-label">LATEST PB</span>
            <span className="hdr-ball" style={{ background: '#cc2222' }}>{lastDraw[0]}</span>
          </div>
        )}
      ...
      <footer className="matrix-footer">
        ...
        <div className="legend-item" style={{ color: '#cc2222', fontWeight: 700 }}>🔴 Powerball Ball Matrix (1–26)</div>
      </footer>
    </div>
  )
}

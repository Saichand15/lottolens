...
const prev = [17, 26, 35, 42, 44]
const actual = [8, 15, 22, 23, 34]
const MAX = 45
...
const prevIdx = draws.findIndex(d => same(d, prev))
const history = prevIdx >= 0 ? draws.slice(0, prevIdx + 1) : draws
...
function formulaList(s) {
  ...
  const defs = [
    ...
    ['S+10', S + 10, 'edge'], ['S-10', S - 10, 'edge'], ['S+ct', S + ct, 'ct'], ['S-ct', S - ct, 'ct'], ['S+ct-1', S + ct - 1, 'ct'], ['S-ct+1', S - ct + 1, 'ct'], ['ct-S', ct - S, 'ct'],

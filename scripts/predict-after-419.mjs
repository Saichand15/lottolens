...
const latestGiven = [2, 9, 15, 21, 25]
...
const shape = shapeReplay(baseDraws, latestGiven)

const combined = new Map()
pred.results.slice(0, 35).forEach((r, idx) => add(combined, r.number, Math.max(0, 92 - idx * 2.2), `hybrid#${idx+1}`))
formulaRank.slice(0, 35).forEach(r => add(combined, r.n, r.pts * 1.28, `formula:${r.pts}`))
shape.ranked.slice(0, 25).forEach((r, idx) => add(combined, r.n, Math.max(0, 62 - idx * 1.6), `shape#${idx+1}`))

// Regime read: 21200 is very low. Historical behavior often rebounds to 30s/40s but keeps one/two low-mid anchors.
if (zoneSig(latestGiven) === '21200') {
  ;[
    [3, 18, 'low hold'], [7, 20, 'low return'], [8, 18, 'low return'], [10, 18, 'boundary return'],
    [14, 18, 'boundary return'], [16, 16, 'boundary return'], [22, 18, 'mid hold'], [23, 16, 'mid hold'], [26, 15, 'mid push'],
    [31, 18, 'high rebound'], [33, 20, 'high rebound'], [34, 18, 'high rebound'], [36, 16, 'high rebound'], [40, 14, 'edge rebound'], [42, 12, 'edge rebound']
  ].forEach(([n, w, why]) => add(combined, n, w, `shape21200:${why}`))
}
...
console.log('\nFORMULA TOP 25:')
formulaRank.slice(0,25).forEach((r,i)=>console.log(`${String(i+1).padStart(2)} ${String(r.n).padStart(2)} pts=${r.pts} ${r.why.slice(0,6).join(' | ')}`))
console.log('\nSHAPE CASES:')
shape.cases.slice(0,8).forEach(c => console.log(`D${c.drawNum} ${c.d.join(',')} -> ${c.next.join(',')} sim=${c.sim.toFixed(1)} sig=${c.sig}`))
console.log('\nCOMBINED TOP 30:')
...
seqs.push(make([best(1,12), best(13,19), best(20,29), best(30,34), best(35,45)]))
seqs.push(make([7,16,22,33,40]))
seqs.push(make([3,14,26,31,42]))
console.log('\nSUGGESTED SEQUENCES:')

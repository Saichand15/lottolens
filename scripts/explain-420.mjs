...
const previous = [2, 9, 15, 21, 25]
const actual = [1, 3, 15, 20, 27]
const historyBefore = [...baseDraws, previous]
const MAX = 45
...
function shapeReplay(history, seeds) {
  ...
  return cases.sort((a,b)=>b.hits-a.hits || b.sim-a.sim).slice(0,12)
}
...
actual.forEach(n => {
  ...
  if (f) console.log('  formulas:', f.why.slice(0,10).join(' | '))
  if (h) console.log('  hybrid:', h.reasons.slice(0,10).join(' | '))
})
console.log('\nFormula top25:', formulaRank.slice(0,25).map(r=>r.n).join(','))
console.log('\nSimilar historical cases hitting actuals:')
shapeReplay(baseDraws, previous).forEach(c => console.log(`D${c.drawNum} ${c.d.join(',')} -> ${c.next.join(',')} hits=${c.hits} sim=${c.sim.toFixed(1)} sig=${c.sig}`))

...
const previous = [22, 27, 32, 34, 39]
const actual = [3, 4, 11, 17, 20]
...
actual.forEach(n => {
  ...
  if (sr) console.log('  first paths:', sr.paths.slice(0,10).map(p => `${p.type}${p.from ? ':'+p.from+'->'+n : ''}${p.seed ? ':seed'+p.seed : ''}${p.dir ? ':'+p.dir : ''}${p.step ? ':s'+p.step : ''}`).join(' ; '))
  if (lr) console.log('  laser first:', lr.details.slice(0,8).map(d => `${d.type}@${d.seed}/${d.dir}/s${d.step}->${n}`).join(' ; '))
})

console.log('\nTop laser:', agent.laser.slice(0,30).map(r=>`${r.number}#${r.rank}:${r.score}`).join(', '))
console.log('\nTop spider:', agent.spider.slice(0,30).map(r=>`${r.number}#${r.rank}:${r.score}`).join(', '))
console.log('\nShape options:', agent.shapeOptions.slice(0,12).map(s=>`${s.signature}:${s.score.toFixed(1)}`).join(', '))
